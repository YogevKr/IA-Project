...
public class LuceneIndexToWekaDataSet {
    ...
    public Instances convertLuceneToWekaClassification(String wekaFileName, String indexFile) throws FileNotFoundException, IOException {
        ...
        RandomAccessFile fichero = new RandomAccessFile(nuevo, "rw");
        fichero.writeBytes("@RELATION " + nuevo.getName() + doubleLine);
        ...
        for (int j = 0; j < terminos.length; j++) {
            fichero.writeBytes("@ATTRIBUTE " + terminos[j].getKey() + " NUMERIC" + "\n");
        }

        fichero.writeBytes("@ATTRIBUTE class {");
        for (int j = 0; j < labelReady.length - 1; j++) {
            fichero.writeBytes(labelReady[j] + ",");
        }
        fichero.writeBytes(labelReady[labelReady.length - 1] + "}" + doubleLine);

        fichero.writeBytes("@DATA\n");
            ...
            if (!reader.isDeleted(pos)) {
                ...
                if (vector != null) {
                    ...
                    for (int j = 0; j < escribir.length; j++) {
                        fichero.writeBytes(escribir[j].getKey() + " " + escribir[j].getValue() + ",");
                    }

                    fichero.writeBytes(terms.size() + " " + searcher.doc(pos).getField(classF).stringValue() + "}\n");
                }
        ...
        //close files
        closeReaders(fichero, searcher, reader);
    ...
    public Instances convertLuceneToWekaRegression(String wekaFileName, String indexFile) throws FileNotFoundException, IOException {
        ...
        fichero.writeBytes("@ATTRIBUTE class REAL [0.0,");

        fichero.writeBytes((labels.size() - 1) + ".0]" + doubleLine);
        ...
        for (int pos = 0; pos < searcher.maxDoc(); pos++) {
            ...
            if (!reader.isDeleted(pos)) {
                ...
                if (vector != null) {
                    ...
                    fichero.writeBytes(terms.size() + " " + labels.get(searcher.doc(pos).getField(classF).stringValue()) + ".0}\n");
                }
     ...
     *
     * @param fichero The file to use.
     * @param searcher The searcher to use.
     * @param reader The reader to use.
     * @throws IOException The exception that can be launched.
     */
    public static void closeReaders(RandomAccessFile fichero, IndexSearcher searcher, IndexReader reader) throws IOException {
        fichero.close();
        searcher.close();
        reader.close();
    }
